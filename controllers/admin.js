    ...
    try {
        if (!req.body.name) {
            ...
            pool.getConnection(function (err, connection) {
                ...
                // Use the connection
                let sql = `set @_returnValue = 0;
                call iot.new_admin_put(${req.params.id}, '${req.body.name}', '${req.body.mobile}', '${req.body.email}', '${req.body.address}', '${req.body.company}', '${req.body.roleId}', @_returnValue);
                select @_returnValue;`;
    ...
    try {
        ...
        if (!req.body.token) {
            ...
            pool.getConnection(function (err, connection) {
                ...
                // Use the connection
                let sql = `set @_returnValue = 0;
                call iot.new_admin_verify('${Buffer.from(req.body.token, 'base64').toString('binary')}', @_returnValue);
                select @_returnValue;`
    ...
    try {
        if (!req.body.email) {
            ...
            pool.getConnection(function (err, connection) {
                ...
                // Use the connection
                let sql = `set @_returnValue = 0;
                call iot.new_admin_login('${req.body.email}', '${req.body.password}', @_returnValue);
                select @_returnValue;`
...
exports.AdminList = (req, res) => {
    try {
        pool.getConnection(function (err, connection) {
            ...
            // Use the connection
            let sql = `call iot.new_admin_get_ByRoleId('${req.params.id}')`;
            connection.query(sql, true, (error, results) => {
                connection.release();
                if (error) {
